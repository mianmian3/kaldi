...
text_union=data/local/train/text_union
lexicon=data/local/dict/lexicon.txt

lm_data=$1
cat ${text} > ${text_union}
cat ${lm_data} | awk '{print "#####",$0}' >> ${text_union}


for f in "$text" "$lexicon" "$lm_data"; do
...
cat $text_union | awk -v lex=$lexicon 'BEGIN{while((getline<lex) >0){ seen[$1]=1; } }
